...
const PhotoStripPage: React.FC = () => {
	...
	const generatePhotoStrip = (targetCanvas: HTMLCanvasElement, scale: number = 1) => {
		...
		if (
			photoStripData.background.type === "image" &&
			...
			bgImagePromise = new Promise<HTMLImageElement | null>((resolve) => {
				...
				img.onload = () => resolve(img);
				img.onerror = () => resolve(null);
				img.src = photoStripData.background.url || photoStripData.background.imageUrl || "";
			});
		}

		// 加载叠加层图片（如果有）
		let overlayImagePromise = Promise.resolve(null);
		if (photoStripData.background.overlayUrl) {
			overlayImagePromise = new Promise<HTMLImageElement | null>((resolve) => {
				const img = new Image();
				img.onload = () => resolve(img);
				img.onerror = () => resolve(null);
				img.src = photoStripData.background.overlayUrl;
			});
		}

		Promise.all([...loadImages, ...loadDecorations, bgImagePromise, overlayImagePromise]).then(
			(loadedAssets) => {
				if (loadedAssets.length === 0) return;

				const photoCount = photoStripData.photos.length;
				const decorationCount = photoStripData.decoration
					? photoStripData.decoration.length
					: 0;

				const loadedPhotos = loadedAssets.slice(0, photoCount) as HTMLImageElement[];
				const loadedDecorations = loadedAssets.slice(
					photoCount,
					photoCount + decorationCount
				) as (HTMLImageElement | null)[];
				const bgImage = loadedAssets[
					photoCount + decorationCount
				] as HTMLImageElement | null;
				const overlayImage = loadedAssets[
					photoCount + decorationCount + 1
				] as HTMLImageElement | null;
				...
				// 根据背景类型绘制
				if (photoStripData.background.type === "color" || !photoStripData.background.type) {
					// 1. 先绘制背景颜色
					...
					// 3. 绘制装饰和文本
					drawDecorations();
				} else if (photoStripData.background.type === "image") {
					// 1. 先绘制背景图片
					if (bgImage) {
						...
						// 如果背景图片加载失败，使用背景颜色
						ctx.fillStyle =
							photoStripData.background.color || selectedColor || "#FFFFFF";
						ctx.fillRect(0, 0, canvas.width, canvas.height);
					}
					...
					// 3. 绘制叠加层图片
					if (overlayImage && photoStripData.background.overlayUrl) {
							...
							if (
								...
								if (pos.borderRadius) {
									...
									drawRoundedRect(
										...
										pos.borderRadius * scale + borderWidth
									);
									ctx.fill();
								} else {
									// 普通矩形边框
		...
		if (customText && photoStripData && photoStripData.text?.content !== customText) {
			updateText({
				...
				position: {
					...
					y: photoStripData.canvasSize.height
						? photoStripData.canvasSize.height - 100
						: 1500,
				},
			});
		}
	...
	const handleShare = async () => {
		...
		try {
			...
			if (navigator.share) {
				...
				toast.info(
					"Sharing not supported on this browser. Photo strip downloaded instead."
				);
			}
		} catch (error) {
					...
					{processedData.hasValidPhotos && processedData.processedPhotos.length > 0 ? (
							...
							<div className="flex flex-col items-center">
								<div className="mb-4 relative mx-auto">
									<PhotoStrip
										...
										filter={
											photoStripData?.photoBoothSettings?.filter || "Normal"
										}
										...
										{colors.map((color, index) => (
											<button
												...
												className={`w-10 h-10 rounded-full border ${
													selectedColor === color
														? "ring-2 ring-offset-2 ring-idol-gold"
						...
						<div className="max-w-4xl mx-auto">
							...
							<MultiPhotoUpload
								...
								aspectRatio={
									currentTemplate?.photoBoothSettings.aspectRatio || "4:3"
								}
							/>
